...
class DiscordCog(commands.Cog):
    ...
    async def user(
        ...
        view = LinkButton(
            ctx=ctx,
            buttons={
                "Download Avatar": user.display_avatar.url,
                "Download Banner": user.banner.url if user.banner else None,
            },
        )
    ...
    async def server(
        ...
        if ctx.guild.banner:
            ...
            view = LinkButton(
                ctx=ctx,
                buttons={
                    "Download Icon": ctx.guild.icon.url,
                    "Download Banner": ctx.guild.banner.url,
                },
            )
        else:
    ...
    async def emote(
        self,
        ctx,
        emoji: Option(str, "The emoji you'd like to download"),
        hidden: Option(
            bool, "Show results in an ephemeral message", required=False
        ) = False,
    ):
